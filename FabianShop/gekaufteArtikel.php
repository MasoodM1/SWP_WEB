...
<body>
    ...
    <div class="container mt-5">
        <div id="artikeldiv" class="row">
            ...
            $sql = "SELECT ArtName, Bild, Preis, Anzahl, date(Zeitpunkt) as Datum from gekaufteArtikel";
            $result = mysqli_query($conn, $sql);
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<h5>Preis: " . $row["Preis"] . "€</h5>";
                    echo "<h5>Stück: " . $row["Anzahl"] . "</h5>";
                    echo "<h5>Gekauft am: " . $row["Datum"] . "</h5>";
                    echo "</div>";
                    echo "</div>";
                }
            }
